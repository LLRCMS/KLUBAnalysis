...
namespace reweight {
  ...
  public:
	...
	MC_distr_ = new TH1D("MC_distr","MC dist",NBins,-0.5, float(NBins)-0.5);
	Data_distr_ = new TH1D("Data_distr","Data dist",NBins,-0.5, float(NBins)-0.5);

	weights_ = new TH1D("luminumer","luminumer",NBins,-0.5, float(NBins)-0.5);
	TH1* den = new TH1D("lumidenom","lumidenom",NBins,-0.5, float(NBins)-0.5);
