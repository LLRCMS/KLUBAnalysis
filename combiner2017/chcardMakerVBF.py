...
def hackTheCard(origName,destName) :
    ...
    for line in infile :
        ...
        if line.startswith('shapes') and f[1]  == "VBFHH_bbtt" :
...
def  writeCard(input,theLambda,select,region=-1) :
	...
	else : variables.append('BDToutSM_kl_1')
	...
	if region < 0 :
		...
		for isy in range(len(syst.SystNames)) :
			...
			if "ggHH" in syst.SystNames[isy] :continue
...
parseOptions()
if(opt.config==""): configname = "../config/analysis_"+opt.channel+"_VBF.cfg"
