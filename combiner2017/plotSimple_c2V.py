...
def getExpValue(c2V): 
    ...
    return ((0.869+31.43968+16.5089*c2V*c2V-8.55784+5.46784*c2V-44.0016*c2V)*0.073)
...
for ipt, c2V in enumerate(C2V):
    fName = 'cards_Combined_2019_10_11_VBFsig_DNN_2Dcount/VBFHH_bbtt_CV_1.0_C2V_{0}_C3_1.0BDToutSM_kl_1/out_Asym_VBFHH_bbtt_CV_1.0_C2V_{0}_C3_1.0_noTH.log'.format(c2V)
    #fName = 'cards_TauTau_2019_10_11/ggHH_bbtt{0}VBFtightBDToutSM_kl_{1}/out_Asym_ggHH_bbtt{0}_noTH.log'.format(lambdas[ipt], klval[ipt])
...
myFunc =  ROOT.TF1("myFunc","(0.869+31.43968+16.5089*x*x-8.55784+5.46784*x-44.0016*x)*0.073", xmin,xmax);
...
for i in range(nP) : 
    ...
    Graph_syst_Scale_y=(getExpValue(xmin+(i*1.)/10.)) 
    Graph_syst_Scale_x_err=(0)

    Graph_syst_Scale_y_errup=(  (0.9+31.4+16.5*(xmin+(i*1.)/10.)*(xmin+(i*1.)/10.)-8.6+5.5*(xmin+(i*1.)/10.)-44*(xmin+(i*1.)/10.))*0.053*0.073)
    Graph_syst_Scale_y_errdown=(  (0.9+31.4+16.5*(xmin+(i*1.)/10.)*(xmin+(i*1.)/10.)-8.6+5.5*(xmin+(i*1.)/10.)-44*(xmin+(i*1.)/10.))*0.067*0.073)
...
#    Graph_syst_Scale2_y=(getExpValue(2*(xmin+(i*1.)/10.)))
...
#
#qgraph2.SetLineColor(ROOT.kRed+1);
#graph2.SetLineWidth(2);


hframe = ROOT.TH1F('hframe', '', 100, -1, 3)
hframe.SetMinimum(0.01)
hframe.SetMaximum(10000000)
...
c1.Print("combined_c2Vscan_DNN_band.pdf", 'pdf')
# for m in masses :
