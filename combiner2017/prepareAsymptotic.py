...
if __name__ == "__main__":
    ...
    #scriptFile.write('combine -M AsymptoticLimits %s/comb.root -m %d -n %s_forLim_noTH --freezeNuisanceGroups QCDscale_ggHH,pdf_ggHH,HH_BR_Hbb,HH_BR_Htt &> out_Asym_%s_noTH.log \n' % (jobsDir,opt.m,opt.n,opt.n))
    scriptFile.write('combine -M AsymptoticLimits %s/comb.root -m %d -n %s_forLim_noTH --freezeNuisanceGroups theory &> out_Asym_%s_noTH.log \n' % (jobsDir,opt.m,opt.n,opt.n))
    ...
    os.system("/home/llr/cms/amendola/t3submit_el7 -short \'%s/runJob_Asym_%s.sh\'"%(jobsDir,opt.n))
