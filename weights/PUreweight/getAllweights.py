...
for i, row in fileNames.iterrows():
    ...
    name  = row["name"]
    command = "root -l -b \"ratioPU.C(0, 50000000, \\\""+filepath+"\\\", \\\""+name+"\\\")\" -q ; root -l \"makeRatio.C(\\\""+name+"\\\")\" > outputs/"+name+".cc -q"

    print command
    os.system(command)
    os.system("rm "+name+".root")
    with  open("outputs/"+name+".cc", 'r') as fin:
        data = fin.read().splitlines(True)
    with open("outputs/"+name+".cc", 'w') as fout:
