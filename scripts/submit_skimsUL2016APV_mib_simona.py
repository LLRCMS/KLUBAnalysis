...
bkg_map = {
    ...
    "EWKWMinus2Jets_WToLNu_M-50"      : "-n 50  -x 20.25",
    "EWKZ2Jets_ZToLL_M-50"            : "-n 50  -x 3.987",

    # --- Single Top ---
    "ST_tW_antitop_5f_inclusiveDecays"        : "-n 50  -x 35.85",
    ...
    # --- TTX ---
    "ttHToNonbb_M125"                 : "-n 50  -x 0.5071   -y 0.423    --ttHToNonBB True",
    "ttHTobb_M125"                    : "-n 50  -x 0.5071   -y 0.577",
    "ttHToTauTau_M125"                : "-n 50  -x 0.5071   -y 0.0632",
...
for label in bkg_map:
    ...
    for f in bkg_files:
        if '%s.txt'%label == f:
            ...
            bkg_map[label] += ' -i %s/%s  -o  %s/SKIM_%s '%(inputDir_bkg, f, skimDir, label)
...
for label in sig_map:
    ...
    for f in sig_files:
        if '%s.txt'%label == f:
            ...
            sig_map[label] += ' -i %s/%s  -o  %s/SKIM_%s '%(inputDir_sig, f, skimDir, label)
...
for label in data_map:
    ...
    for f in data_files:
        if ('%s'%label) in f:
            ...
            data_map[label] += ' -i %s/%s  -o  %s/SKIM_%s '%(inputDir_data, f, skimDir, label)
