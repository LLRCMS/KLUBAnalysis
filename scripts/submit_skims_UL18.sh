...
DATA_MAP=(
    ...
    # ["SingleMuon__Run2018A"]="-n 800 -q short"
...
# MASSES=("250" "260" "270" "280" "300" "320" "350" "400" "450" "500" "550" "600" "650" "700" "750" "800
...
for ds in ${DATA_LIST[@]}; do
	for mass in ${MASSES[@]}; do
		...
		if [[ ${sample} =~ ${SEARCH_SPACE} ]]; then
			...
			run_skim -n 5 -i ${SIG_DIR} --sample ${sample} -x 1. -q long
		fi
	done
done
...
MC_MAP=(
    ...
    # ["TTToSemiLeptonic"]="-n 3000 -x ${SemiLepXSec} -q short"
    ...
    # ["DYJets.+_M-50_T.+amc"]="-n 1000 -x 6077.22 -g ${STITCHING_ON} --DY 0 -q short" # inclusive NLO
    ...
    # ["DYJetsToLL_LHEFilterPtZ-100To250"]="-n 600 -x 92.24   -g ${STITCHING_ON} --DY 0 -q short"
    ...
    # ["DYJetsToLL_0J"]="-n 600 -x 4867.28  -g ${STITCHING_ON} --DY 0 -q short"
    # ["DYJetsToLL_1J"]="-n 600 -x 902.95   -g ${STITCHING_ON} --DY 0 -q short"
    # ["DYJetsToLL_2J"]="-n 600 -x 342.96   -g ${STITCHING_ON} --DY 0 -q short"
