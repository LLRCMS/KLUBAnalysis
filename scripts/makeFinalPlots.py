    ...
    for idx, name in enumerate(inputs):
        if name.startswith(prefix) and name.endswith(prefix):
            ...
            if doOverflow:
                ...
                break
    if histo is None:
        mes = 'Check the input histograms do have a TH1 called {}.'
...
if __name__ == "__main__" :
    ...
    parser.add_argument('--detailed', action='store_true', default=False,
                        help='Plot all background contributions separately.')
    ...
    if args.detailed:
        bkgColors.update({'EWK':     (ROOT.kSpring-7, ROOT.kSpring-7),
                          'HH':      (ROOT.kOrange,   ROOT.kOrange),
                          'VV':      (ROOT.kYellow,   ROOT.kYellow),
                          'VVV':     (ROOT.kSpring,   ROOT.kSpring),
                          'TTX':     (ROOT.kTeal-7,   ROOT.kTeal-7),
                          # 'TTHX':    (ROOT.kAzure+4,  ROOT.kAzure+4),
                          'TW':      (ROOT.kCyan,     ROOT.kCyan),
                          'singleT': (ROOT.kAzure,    ROOT.kAzure),
                          'ZH':      (ROOT.kBlue,     ROOT.kBlue),
                          'WH':      (ROOT.kBlue-7,   ROOT.kBlue-7),
                          'ttH':     (ROOT.kMagenta,  ROOT.kMagenta),
                          'ggH':     (ROOT.kPink,     ROOT.kPink),
                          'qqH':     (ROOT.kRed+2,    ROOT.kRed+2),
                          })
    else:
        bkgColors.update({'other': (col.GetColor("#F89C20"), col.GetColor("#F89C20"))})
    ...
    if args.detailed:
        hEWK       = getHisto('EWK', *hopt)
        hHH        = getHisto('HH', *hopt)
        hVV        = getHisto('VV', *hopt)
        hVVV       = getHisto('VVV', *hopt)
        hTTX       = getHisto('TTX', *hopt)
        # hTTHX      = getHisto('TTHX', *hopt)
        hTW        = getHisto('TW', *hopt)
        hsingleT   = getHisto('singleT', *hopt)
        hZH        = getHisto('ZH', *hopt)
        hWH        = getHisto('WH', *hopt)
        httH       = getHisto('ttH', *hopt)
        hggH       = getHisto('ggH', *hopt)
        hqqH       = getHisto('qqH', *hopt)
    ...
    bkgStack.GetYaxis().SetTitleOffset(1.59)
    ...
    ################## LEGEND ######################################
    legxmin = 0.53 if args.detailed else 0.73
    legymin = 0.6 if not args.lymin else args.lymin
    legsize, legfont = 20, 43
    legBkg = ROOT.TLegend(legxmin, legymin, 0.9, 0.9)
    if args.detailed:
        legBkg.SetNColumns(3);
    ...
    if not args.nosig:
        legSig = ROOT.TLegend(legxmin-0.25, legymin, legxmin, 0.90)
    elif args.nosig and not args.nodata:
        legSig = ROOT.TLegend(legxmin-0.15, 0.85, legxmin, 0.90)
    ...
    extraTextSize = 0.8 * cmsTextSize # for the "preliminary"
    ...
    if args.ratio and not args.nodata:
        ...
        hRatio.SetMaximum(1.59) #default value
