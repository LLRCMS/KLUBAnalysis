...
for m in masses:
    ...
    gr_btag.SetPoint(gr_btag.GetN(), m, n_btag/n_all)
    gr_mcut.SetPoint(gr_mcut.GetN(), m, n_mcut/n_all)
    if gr_BDT : gr_BDT.SetPoint(gr_BDT.GetN(), m, n_BDT/n_all)
...
fOut = ROOT.TFile('effPlots_%s_resonant_%s.root' % (channel, stype), 'recreate')
gr_trg.Write()
gr_tautau.Write()
gr_3lep.Write()
gr_2jets.Write()
