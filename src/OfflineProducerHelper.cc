...
bool OfflineProducerHelper::pairPassBaseline (bigTree* tree, int iPair, TString whatApply, bool debug)
{
    ...
    if (pairType == MuHad)
    {
        ...
        leg2 = tauBaseline (tree, dau2index, 20., 2.3, aeleVVVLoose, amuTight, tauIso, whatApply, debug); //Davide: deepTau
   }
    ...
    if (pairType == EHad)
    {
        ...
        leg2 = tauBaseline (tree, dau2index, 20., 2.3, aeleVTight, amuVLoose, tauIso, whatApply, debug); //Davide: deepTau
    }
 ...
 */
    ...
    bool isoS =  checkBit(tree->tauID->at(iDau),getTAUidNumber("byVVVLooseDeepTau2017v2VSjet")) || byp_isoS;
    if (whatApply.Contains ("InvertIzo")) isoS = !isoS ;
...
int OfflineProducerHelper::getBestPairHTauTau (bigTree* tree, TString whatApply, bool debug)
{
  ...
  for (unsigned int ipair = 0; ipair < tree->indexDau1->size(); ++ipair)
  {
    ...
    float iso1 = tree->daughters_byDeepTau2017v2VSjetraw->at(t_firstDaughterIndex);
    float iso2 = tree->daughters_byDeepTau2017v2VSjetraw->at(t_secondDaughterIndex);
  ...
  for (uint ipair = 0; ipair < vPairs.size(); ++ipair)
  {
    ...
    bool leg1 = tauBaseline (tree, dau1index, 20., 2.1, aeleVVVLoose, amuVLoose, 99999., whatApply, debug);//Davide: deepTau
    bool leg2 = tauBaseline (tree, dau2index, 20., 2.1, aeleVVVLoose, amuVLoose, 99999., whatApply, debug);//Davide: deepTau
