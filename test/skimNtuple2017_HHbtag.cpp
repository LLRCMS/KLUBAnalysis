...
int main (int argc, char** argv)
{
  ...
  tau_trigger::SFProvider * tauTrgSF_vbf   = new tau_trigger::SFProvider("weights/trigger_SF_Legacy/2017/2017_tauTriggerEff_DeepTau2017v2p1.root", "ditauvbf", "Medium");
  ...
  for (Long64_t iEvent = 0 ; true ; ++iEvent) 
    {
      ...
      if (isHHsignal && isHHNLO && HHrewType != kNone)
      {
        ...
        if (fileName.find("node_cHHH0")    != std::string::npos) addWeight = 1. / 58456.4 / 4.; // computed with hhReweight/MakeMap_getNLOweights.cpp
        if (fileName.find("node_cHHH1")    != std::string::npos) addWeight = 1. / 26052.8 / 4.; // computed with hhReweight/MakeMap_getNLOweights.cpp
        if (fileName.find("node_cHHH2p45") != std::string::npos) addWeight = 1. / 10948.3 / 4.; // computed with hhReweight/MakeMap_getNLOweights.cpp
        if (fileName.find("node_cHHH5")    != std::string::npos) addWeight = 1. / 76938.6 / 4.; // computed with hhReweight/MakeMap_getNLOweights.cpp
        if (DEBUG)
        {
