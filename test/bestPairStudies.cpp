...
int main()
{
    ...
    HistoManager* pairCounter = new HistoManager ("pairCounter");
    for (int i = 0; i < nsel; i++)
    {
        ...
        */
        ...
        for (int iSel = 0; iSel < nsel; iSel++)
        {
            ...
            for (int iMoth = 0; iMoth < tree->mothers_px->size(); iMoth++)
            {
                ...
                // just restrict to pairs of this MC category
                if (pairType != MCtype) continue;
...
// TO DO: check when the pair selected with an algorithm is really the good best pair, i.e. the one where I matched with the gen particles
            ...
            pairCounter -> Get2DHisto (sel[iSel].Data()) -> Fill (MCtype, nPairsPassing);
        }
    }
