  ...
  for (Long64_t iEvent = 0 ; true ; ++iEvent)
	{
	  ...
	  if (debugEvents.size() > 0 && DEBUG == false)
		continue;
	  ...
	  if (HTMax > 0 and ( theBigTree.lheHt > HTMax or theBigTree.lheHt < HTMin ))
		continue;

	  // directly reject events I want to remove in W+Jets stitching on njets
	  if (NjetRequired >= 0 and theBigTree.lheNOutPartons != NjetRequired)
		continue;

	  // For ggHH NLO samples only: if the genWeight is > 0.5 the event should be rejected
	  if (isHHNLO and abs(theBigTree.aMCatNLOweight) > 0.5)
		continue;
