...
*/ 
// computed for Legacy2016, 23 Jan 2020
const float stitchWeights [][5] = {
  {4.53527779613 , 0.0 , 0.0 , 0.0 , 0.0},
  {0.519441196235 , 0.518921151443 , 0.0 , 0.0 , 0.0},
  {0.182019457321 , 0.182070146792 , 0.0305665940278 , 0.0 , 0.0},
  {0.236236292658 , 0.23657566063 , 0.0313131091642 , 0.0309178123659 , 0.0},
  {0.267766889275 , 0.267878327658 , 0.0379689601745 , 0.0310389796023 , 0.0401280816561}
};
...
int main (int argc, char** argv)
{
  ...
  TauIDSFTool * Deep_antiMu_loose      = new TauIDSFTool("2018ReReco","DeepTau2017v2p1VSmu" ,"Loose");    // for DeepTauv2p1 vs mu Loose
  ...
  for (Long64_t iEvent = 0 ; true ; ++iEvent) 
      ...
      if(pType == 2)                //1st tau
	{
	  ...
	  if(DEBUG)
	  {
	     ...
	     cout<<"B1"<<endl;
	     cout<<"(pt,eta,phi,e) "<<tlv_firstBjet.Pt()<<","<<tlv_firstBjet.Eta()<<","<<tlv_firstBjet.Phi()<<","<<tlv_firstBjet.E()<<endl;
	     cout<<"B2"<<endl;
	     cout<<"(pt,eta,phi,e) "<<tlv_secondBjet.Pt()<<","<<tlv_secondBjet.Eta()<<","<<tlv_secondBjet.Phi()<<","<<tlv_secondBjet.E()<<endl;
	     cout<<"MET"<<endl;
